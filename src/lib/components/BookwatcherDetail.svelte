...
<div class="bw-container">
  ...
  <div class="bw-card">
    <div class="bw-card-title">Also labeled by</div>
    <div class="bw-other-chips">
      {#each otherLabels as ol}
      {@const olb = labelers[ol.labeler]}
      <a class="bw-other-chip" href={ol.action_url} style="background: {olb.bg}; color: {olb.color}; border: 1px solid {olb.color}33">
        {olb.name}
      </a>
      {/each}
    </div>
...
<style>
  ...
  .bw-nav-right { margin-left: auto; font-size: 13px; }
  ...
  .design-annotation { margin-top: 32px; padding: 14px 18px; border: 1.5px dashed #cbd5e1; border-radius: 6px; background: #fefce8; font-size: 11.5px; color: #78716c; position: relative; line-height: 1.65; }
