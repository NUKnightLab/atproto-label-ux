...
* A WebTiles case where instead of the buttons we see now, the AppView uses the labeler's preferred WebTile (or inlay.at) "template" to present an even richer experience than the simple CTA button. (Does WebTiles allow links to other domains? Kinda remember maybe not.)
